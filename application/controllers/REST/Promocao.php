            ...
            if (!$this->post('descricao') || (!$this->post('prcnt_desconto') && !$this->post('desconto_fixo')) || !$this->post('data_ini') || !$this->post('data_fim')) {
                ...
                if ($this->Promocao_model->getEmpresa($id)) {
                    ...
                    if ($retorno > 0) {
                        ...
                        $this->set_response('Falha ao cadastrar promoção', REST_Controller_Definitions::HTTP_BAD_REQUEST);
                        return;
                    }
                }
                $this->set_response('Empresa não encontrada', REST_Controller_Definitions::HTTP_BAD_REQUEST);
                return;
            }
        }
    }
